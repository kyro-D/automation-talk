...
Give script executable permissions with `chmod +x {path to script}`. Note that mac considers files in `~/Documents` as private so you will encouter permissions errors if you try to place the script here. I got around this by making a folder under my user directory `~/Cron-Job-Scripts`. 
